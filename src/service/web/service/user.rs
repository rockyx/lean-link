use crate::{
    AppState,
    database::users,
    service::web::middleware::jwt,
    utils::datetime::{local_time, local_time_option},
};
...
async fn login(
...
) -> actix_web::Result<web::Json<super::WebResponse<UserLoginResponse>>> {
    let db_conn = &app_state.db_conn;

    let user = match users::find_user_by_name(db_conn, req.username.clone()).await {
        Ok(Some(user)) => user,
        Ok(None) => {
            return Ok(super::WebResponse::with_error_code(
                super::ErrorCode::InvalidUsernameOrPassword,
            )
            .into());
        }
        Err(e) => {
            tracing::error!(error = ?e);
            return Ok(super::WebResponse::with_error_code(super::ErrorCode::InternalError).into());
        }
    };
    ...
    Ok(super::WebResponse::with_result(resp).into())
}
